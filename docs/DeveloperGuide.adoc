...
==== Add Profile Page
The profile page parameter is facilitated by the class `ProfilePage` of which the class `Person` consists. As profile page is an optional paraeterm, a `Person` can be linked to 0 or 1 `ProfilePage` object. +
...
* `UI Component:` +
`PersonCard`, which resides in UI component, create a label for profile page parameter:
[source, java]
----
@FXML
...
=== delete: deletes a list of persons with specified indices
