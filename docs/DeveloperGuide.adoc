...
In order to make `select` command on a `Person` load that person's `ProfilePage` object if existent, we modify method `handlePersonPanelSelectionChangedEvent()` in `BrowserPanel` that is in charge of updating a person panel: +
[source, java]
----
